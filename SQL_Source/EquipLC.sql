...
CREATE TRIGGER [dbo].[EquipLC] ON [dbo].[Equipped]
AFTER UPDATE
AS
BEGIN
IF UPDATE(LightCone)
BEGIN
	...
	SELECT @LightCone=inserted.LightCone FROM inserted WHERE inserted.LightCone IS NOT NULL AND EXISTS(SELECT * FROM deleted WHERE deleted.Concept=inserted.Concept AND deleted.LightCone IS NULL);
	SELECT @Character=inserted.Concept FROM inserted;
	IF(@LightCone IS NOT NULL)
	BEGIN
	SELECT @SuperimposeLevel=LightCones.SuperimposeLevel FROM LightCones WHERE LightCones.Concept=@LightCone;
	...
	BEGIN
	    UPDATE Equipped
		SET Equipped.SuperimposeLevel=@SuperimposeLevel WHERE Equipped.Concept=@Character;
		UPDATE Equipped
...
ALTER TABLE [dbo].[Equipped] ENABLE TRIGGER [EquipLC]
GO
