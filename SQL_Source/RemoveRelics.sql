...
CREATE TRIGGER [dbo].[RemoveRelics] ON [dbo].[Equipped]
AFTER UPDATE
AS
BEGIN
IF(UPDATE(Head) OR UPDATE(Hands) OR UPDATE(Body) OR UPDATE(Feet) OR UPDATE(PlanarSphere) OR UPDATE(LinkRope))
BEGIN
     SET NOCOUNT ON;
	 DECLARE @ID smallint;
	 DECLARE @Head smallint;
	 DECLARE @Hands smallint;
	 DECLARE @Body smallint;
	 DECLARE @Feet smallint;
	 DECLARE @PlanarSphere smallint;
	 DECLARE @LinkRope smallint;
	 ...
	 SELECT @Equipped=inserted.Concept FROM inserted;
	 SELECT @Head=deleted.Head FROM deleted WHERE deleted.Head IS NOT NULL AND EXISTS(SELECT Head FROM inserted WHERE inserted.Concept=deleted.Concept AND inserted.Head IS NULL);
	 SELECT @Hands=deleted.Hands FROM deleted WHERE deleted.Hands IS NOT NULL AND EXISTS(SELECT Hands FROM inserted WHERE inserted.Concept=deleted.Concept AND inserted.Hands IS NULL);
	 SELECT @Body=deleted.Body FROM deleted WHERE deleted.Body IS NOT NULL AND EXISTS(SELECT Body FROM inserted WHERE inserted.Concept=deleted.Concept AND inserted.Body IS NULL);
	 SELECT @Feet=deleted.Feet FROM deleted WHERE deleted.Feet IS NOT NULL AND EXISTS(SELECT Feet FROM inserted WHERE inserted.Concept=deleted.Concept AND inserted.Feet IS NULL);
	 SELECT @PlanarSphere=deleted.PlanarSphere FROM deleted WHERE deleted.PlanarSphere IS NOT NULL AND EXISTS(SELECT PlanarSphere FROM inserted WHERE inserted.Concept=deleted.Concept AND inserted.PlanarSphere IS NULL);
	 SELECT @LinkRope=deleted.LinkRope FROM deleted WHERE deleted.LinkRope IS NOT NULL AND EXISTS(SELECT LinkRope FROM inserted WHERE inserted.Concept=deleted.Concept AND inserted.LinkRope IS NULL);
	 IF(@Head IS NOT NULL)
		BEGIN
			SET @ID = @Head;
		END
	 ELSE IF(@Hands IS NOT NULL)
		BEGIN
			SET @ID = @Hands;
		END
	 ELSE IF(@Body IS NOT NULL)
		BEGIN
			SET @ID = @Body;
		END
	 ELSE IF(@Feet IS NOT NULL)
		BEGIN
			SET @ID = @Feet;
		END
	 ELSE IF(@PlanarSphere IS NOT NULL)
		BEGIN
			SET @ID = @PlanarSphere;
		END
	 ELSE IF(@LinkRope IS NOT NULL)
		BEGIN
			SET @ID = @LinkRope;
		END
IF(@ID IS NOT NULL)
  BEGIN
	UPDATE Relics
	SET Equipped = NULL WHERE Relics.ID = @ID;
	SELECT @MainStat=Relics.MainStat FROM Relics WHERE @ID=Relics.ID
	SELECT @MSValue=Relics.MSValue FROM Relics WHERE @ID=Relics.ID
	SELECT @SubStat_1=Relics.SubStat_1 FROM Relics WHERE @ID=Relics.ID
	SELECT @SubStat_2=Relics.SubStat_2 FROM Relics WHERE @ID=Relics.ID
	SELECT @SubStat_3=Relics.SubStat_3 FROM Relics WHERE @ID=Relics.ID
	SELECT @SubStat_4=Relics.SubStat_4 FROM Relics WHERE @ID=Relics.ID
	SELECT @SSValue_1=Relics.SSValue_1 FROM Relics WHERE @ID=Relics.ID
	SELECT @SSValue_2=Relics.SSValue_2 FROM Relics WHERE @ID=Relics.ID
	SELECT @SSValue_3=Relics.SSValue_3 FROM Relics WHERE @ID=Relics.ID
	SELECT @SSValue_4=Relics.SSValue_4 FROM Relics WHERE @ID=Relics.ID
  END
	IF(@ID IS NOT NULL)
	BEGIN
		 IF(@MainStat='HPBoost')
	...
	BEGIN
	 ...
	 SELECT @HeadSet=Relics.Concept FROM Relics,inserted WHERE Relics.ID=inserted.Head
	 SELECT @HandsSet=Relics.Concept FROM Relics,inserted WHERE Relics.ID=inserted.Hands
	 SELECT @BodySet=Relics.Concept FROM Relics,inserted WHERE Relics.ID=inserted.Body
	 SELECT @FeetSet=Relics.Concept FROM Relics,inserted WHERE Relics.ID=inserted.Feet
	 SELECT @PlanarSphereSet=Relics.Concept FROM Relics,inserted WHERE Relics.ID=inserted.PlanarSphere
	 SELECT @LinkRopeSet=Relics.Concept FROM Relics,inserted WHERE Relics.ID=inserted.LinkRope
...
ALTER TABLE [dbo].[Equipped] ENABLE TRIGGER [RemoveRelics]
GO
